...
export const useChatWebSocket = () => {
  ...
  const { activeChatRoomId, setChatRooms, setMessages, setChatRoomsInfo } = useChat()
        ...
        setChatRoomsInfo((prevChatRoomsInfo) => {
          return prevChatRoomsInfo.map((info) => {
            if (info.chatRoomId === parsedMessage.chatRoomId) {
              ...
              } else if (parsedMessage.sender !== 'admin' && parsedMessage.sender !== nickname) {
                return {
                  ...info,
        ...
        setChatRooms((prevChatRooms) => {
          return prevChatRooms.map((info) => {
            if (info.chatRoomId === parsedMessage.chatRoomId) {
              return {
                ...info,
                userInfo: [...info.userResponses, ...parsedMessage.userResponses],
              }
            }
            return info
          })
