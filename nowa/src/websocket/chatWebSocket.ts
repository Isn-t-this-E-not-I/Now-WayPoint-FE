...
export const useChatWebSocket = () => {
  ...
  const connectAndSubscribe = () => {
    ...
    const onConnect = () => {
      stompClient!.subscribe(`/queue/chatroom/${localStorage.getItem('nickname')}`, (message) => handleMessageUser(message));
    };
  ...
  const subscribeToChatRoom = (chatRoomId: number) => {
    if (stompClient) {
      chatRoomSubscription = stompClient.subscribe(`/topic/chatroom/${chatRoomId}`, (message) => handleMessageChatRoom(message));
  ...
  const handleMessageUser = (message: { body: string }) => {
    ...
    switch (parsedMessage.messageType) {
      ...
      case 'CREATE_DUPLICATE':
        ...
        setChatRooms((prevChatRooms) => [...prevChatRooms, newChatRoomDuplicate]);
        break;
      case 'CHAT_LIST':
        const newMessages: ChatMessage[] = parsedMessage.messages.map((msg: any) => ({
          sender: msg.sender,
          content: msg.content,
      ...
      case 'DELETE':
        setChatRooms((prevChatRooms) => {
          ...
          return prevChatRooms.filter((info) => info.chatRoomId !== parsedMessage.chatRoomId);
        });
        break
      case 'ERROR':
