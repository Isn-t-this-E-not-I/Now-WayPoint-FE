...
const FindPasswordPage = () => {
  ...
  const handleFindPassword = async () => {
    try {
      ...
      if (userValid.exists) {
        const verificationResponse = await sendVerificationCode(email, '비밀번호찾기');
  ...
  const handleVerifyCode = async () => {
    if (verificationCode === receivedCode) {
      ...
      const resetResponse = await resetPassword(email, verificationCode, tempPassword);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-white">
      <div className="card w-96 shadow-xl p-5 bg-gray-50">
        ...
        <TextInput type="text" placeholder="아이디 입력" onChange={e => setUserId(e.target.value)} value={userId} className="mb-4" />
        <TextInput type="email" placeholder="이메일 입력" onChange={e => setEmail(e.target.value)} value={email} className="mb-4" />
        {receivedCode && <TextInput type="text" placeholder="인증 코드 입력" onChange={e => setVerificationCode(e.target.value)} value={verificationCode} className="mb-4" />}
