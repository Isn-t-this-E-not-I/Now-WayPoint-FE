  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-white">
      <div className="card w-96 shadow-xl p-5 bg-gray-50">
        ...
        <button className="btn btn-primary mt-4 mb-2" onClick={handleRequestAuthNumber}>인증 코드 받기</button>
        {receivedCode && <TextInput type="text" placeholder="인증 코드 입력" onChange={e => setAuthNumber(e.target.value)} className="mb-4" />}
        {receivedCode && <button className="btn btn-primary mt-2" onClick={handleVerifyAuthNumber}>인증 확인</button>}
        {foundId && <div className="text-green-500 text-sm mt-2">아이디: {foundId}</div>}
