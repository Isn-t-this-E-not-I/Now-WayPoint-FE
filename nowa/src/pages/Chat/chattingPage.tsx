...
const Timestamp = styled.span<{ $isSender: boolean }>`
  font-size: 0.8rem;
  color: #999;
  align-self: ${(props) => (props.$isSender ? 'flex-start' : 'flex-end')};  /* 위치 조정 */
  position: absolute;
  bottom: 5px;
...
const ChattingPage: React.FC = () => {
  ...
  const { chatRooms, messages, setChatRooms, setChatRoomsInfo, setMessages } = useChat()
  const { theme } = useApp()
  ...
  if (chatRoom.userResponses.length === 1) {
    ...
    const otherUser = chatRoom.userResponses.find(user => user.userNickname !== nickname)
  ...
  return (
    <ChatContainer>
      <Header>
        ...
        <ButtonContainer>
          <ActionButton onClick={() => setShowInviteModal(true)}>채팅방에 초대</ActionButton>
