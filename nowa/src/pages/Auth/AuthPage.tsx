...
import { login, register, sendVerificationCode, verifyCode, checkLoginId, sendLoginInfo } from '../../api/userApi';
import TextInput from '../../components/TextInput/textInput';
import { useNavigate } from 'react-router-dom';
import { AiFillEye, AiFillEyeInvisible, AiFillCaretDown, AiFillCaretUp } from 'react-icons/ai';
...
const AuthPage: React.FC = () => {
  ...
  return (
    ...
    <div className="flex justify-end w-full" style={{ marginTop: '0.1rem', marginRight: '42rem' }}>
    <a
        href="#"
        className="mr-8"
        style={{
      ...
      <div className="card w-96 p-5" style={{ marginTop: '3rem', marginLeft: '40rem', minHeight: '590px' }}>
...
const RegisterForm: React.FC<RegisterFormProps> = ({ setActiveTab }) => {
  ...
  const isNicknameValid = (nickname: string) => /^[ㄱ-ㅎ가-힣a-zA-Z0-9]+$/.test(nickname);
  const isLoginIdValid = (loginId: string) => /^[a-zA-Z0-9]{6,12}$/.test(loginId);
  const isPasswordValid = (password: string) => /^[\w!@#$%^&*]{8,12}$/.test(password);
  ...
  const handleSendCode = async () => {
    ...
    if (loginId && email && password && name && nickname) {
      ...
      if (!isPasswordValid(password)) {
        setPasswordMessage('비밀번호는 8~12자의 문자/숫자/기호 조합이어야 합니다.');
        return;
      }
      ...
      try {
        const verificationResponse = await sendVerificationCode(email, '회원가입', '');
        if (verificationResponse.message) {
          setReceivedCode(verificationResponse.message);
          setCodeSentMessage('인증 코드가 이메일로 발송되었습니다. 코드를 확인해주세요.');
  ...
  return (
    <>
      ...
      {loginMessage && (
        <div className={`text-sm mb-2 ${loginMessage.includes('사용 가능한') ? 'text-green-500' : 'text-red-500'}`}>
          {loginMessage}
        </div>
      )}
      <div className="relative w-full mb-2">
        <TextInput
          ...
          value={password}
          className="mb-4"
          style={{ color: 'black', backgroundColor: '#EAF0F7', border: 'none', paddingRight: '2.5rem' }}
        />
        <div
      ...
      {passwordMessage && (
        <div className={`text-sm mb-2 ${passwordMessage.includes('비밀번호는') ? 'text-red-500' : 'text-green-500'}`}>
          {passwordMessage}
        </div>
      )}
      <div className="flex items-center mb-4">
        ...
        <div className="relative w-40">
          <div
            ...
            style={{ backgroundColor: '#EAF0F7', border: 'none', padding: '0.5rem 1rem' }}
      ...
      />
      {nicknameMessage && (
        <div className={`text-sm mb-4 ${nicknameMessage.includes('닉네임에는') ? 'text-red-500' : 'text-green-500'}`}>
          {nicknameMessage}
        </div>
      )}
      <button
